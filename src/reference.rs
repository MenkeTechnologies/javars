...
pub const REFERENCE: &[Entry] = &[
    ...
    (
        ...
        "Operators",
        "name++;   |   a[i]++;   |   obj.f++;",
        "Post-increment, narrowing back to 32 bits when the target is `int`. In value position it evaluates to the value the variable *held*, with the update applied after; the prefix form `++i` updates first and evaluates to the new value.",
    ...
    (
        ...
        "printf-style formatting — a faithful subset of `java.util.Formatter` covering the conversions `d s S f e E g G b B h H x X o c %` and `%n`, the flags `-` `0` `+` `,` `(`, an optional width, an optional `.precision`, and explicit argument indexes (`%2$s`). `%f` rounds HALF_UP on the double's exact value, as Java does. `System.out.printf` is the same formatter with no trailing newline. An unsupported conversion is reported rather than rendered wrong.",
