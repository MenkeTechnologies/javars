//! The `java` binary entry point.
//!
//! Runs a `.java` file on fusevm, or serves an introspection flag
//! (`--version`, `--dump-tokens`/`--dump-ast`/`--disasm`). Errors go to stderr
//! in terse `javars: <reason>` form; nothing else is printed.

use std::process::ExitCode;

fn main() -> ExitCode {
    let cli = match javars::cli::parse(std::env::args().skip(1)) {
        Ok(c) => c,
        Err(e) => return fail(&e),
    };

    if cli.show_version {
        println!("{}", javars::version_banner());
        return ExitCode::SUCCESS;
    }
    if cli.show_help {
        print!("{}", javars::cli::USAGE);
        return ExitCode::SUCCESS;
    }
    ...
    if cli.disasm {
        return finish(javars::disassemble(&src));
    }
    if cli.tiers {
        return match javars::tiers::report(&src) {
            Ok(r) => {
                println!("{r}");
                ExitCode::SUCCESS
            }
...
fn fail(msg: &str) -> ExitCode {
    let msg = msg.strip_prefix("javars: ").unwrap_or(msg);
    eprintln!("javars: {msg}");
    ExitCode::FAILURE
}
