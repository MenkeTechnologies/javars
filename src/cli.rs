//! Command-line parsing for the `java` binary.
//!
//! Slice 1 accepts a single-file invocation plus a small set of introspection
//! flags. The `java` launcher's full option grammar (`-cp`, `-jar`, `-D…`,
//! module flags) grows in later waves; unknown options error rather than being
//! silently ignored.

/// Parsed command line.
#[derive(Debug, Default)]
pub struct Cli {
    /// The `.java` file to run, if any.
    ...
    for a in args {
        match a.as_str() {
            ...
            _ if a.starts_with('-') && cli.file.is_none() => {
                return Err(format!("javars: unrecognized option `{a}`"))
            }
            _ => {
                if cli.file.is_none() {
                    cli.file = Some(a);
...
/// `java --help` text.
pub const USAGE: &str = "\
usage: java [options] <file.java> [args...]
